...
import org.mule.module.magento.processors.CreateProductAttributeMediaMessageProcessor;
...
public class CreateProductAttributeMediaDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(CreateProductAttributeMediaDefinitionParser.class);

    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(CreateProductAttributeMediaMessageProcessor.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the @Processor [create-product-attribute-media] within the connector [magento] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [create-product-attribute-media] within the connector [magento] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.addConstructorArgValue("createProductAttributeMedia");
        ...
        parseProperty(builder, element, "baseFileName", "baseFileName");
